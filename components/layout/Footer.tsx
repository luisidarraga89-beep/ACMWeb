...
export default function Footer() {
  return (
    <footer className="bg-cream border-t border-graphite/10" aria-labelledby="footer-heading">
      ...
      <div className="container-acm">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8 pt-16 pb-12 border-b border-white/8">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <p className="font-display italic text-sm text-navy-deep/65 leading-relaxed mb-1 max-w-[22ch]">
            ...
            <div className="flex items-center gap-2" role="list" aria-label="Redes sociales">
              {SOCIAL_LINKS.map(({ label, href, icon }) => (
                <a key={label} href={href} target="_blank" rel="noopener noreferrer" role="listitem" aria-label={label}
                  className="w-8 h-8 flex items-center justify-center rounded-md bg-navy-deep/5 border border-navy-deep/10 text-navy-deep/50 hover:text-navy-deep hover:bg-navy-deep/10 transition-all duration-200 focus-visible:outline-orange-acm">
                  {icon}
                </a>
              ))}
            </div>
          ...
          {/* Servicios */}
          <div>
            <h3 className="font-sans text-[0.6875rem] font-semibold tracking-[0.1em] uppercase text-navy-deep/40 mb-4">Servicios</h3>
            <ul className="space-y-2.5">
              {FOOTER_SERVICES.map(({ label, href, badge }) => (
                <li key={href}>
                  <Link href={href} className="group flex items-center gap-2 font-sans text-sm text-navy-deep/65 hover:text-navy-deep transition-colors duration-200 focus-visible:outline-orange-acm rounded">
                    <span className="w-1.5 h-1.5 rounded-full bg-orange-acm shrink-0 opacity-80 group-hover:opacity-100 transition-opacity" aria-hidden="true" />
                    {label}
          ...
          {/* Empresa */}
          <div>
            <h3 className="font-sans text-[0.6875rem] font-semibold tracking-[0.1em] uppercase text-navy-deep/40 mb-4">Empresa</h3>
            <ul className="space-y-2.5">
              {FOOTER_COMPANY.map(({ label, href }) => (
                <li key={href}>
                  <Link href={href} className="font-sans text-sm text-navy-deep/65 hover:text-navy-deep transition-colors duration-200 focus-visible:outline-orange-acm rounded">{label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Contacto */}
          <div>
            <h3 className="font-sans text-[0.6875rem] font-semibold tracking-[0.1em] uppercase text-navy-deep/40 mb-4">Contacto</h3>
            <ul className="space-y-3">
              <li className="flex items-start gap-2.5">
                ...
                <a href={`tel:${siteConfig.contact.phone}`} className="font-sans text-sm text-navy-deep/65 hover:text-navy-deep transition-colors duration-200">{siteConfig.contact.phone}</a>
              </li>
              <li className="flex items-start gap-2.5">
                ...
                <a href={`mailto:${siteConfig.contact.email}`} className="font-sans text-sm text-navy-deep/65 hover:text-navy-deep transition-colors duration-200 break-all">{siteConfig.contact.email}</a>
              </li>
              <li className="flex items-start gap-2.5">
            ...
            <a href={siteConfig.social.whatsapp} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 mt-5 font-sans text-sm font-semibold text-orange-acm hover:text-orange-acm/70 transition-colors duration-200 focus-visible:outline-orange-acm rounded">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 py-5">
          <p className="font-sans text-xs text-graphite/50 text-center sm:text-left">
            © {CURRENT_YEAR} <span className="text-orange-acm">ACM</span> Hogares e Inversiones · Bogotá, Colombia
          </p>
