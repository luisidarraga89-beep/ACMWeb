...
export default function Navbar() {
  ...
  // Navbar is always navy — no transparency on light pages
  return (
    <>
      <header
        ...
        style={{
          ...
          boxShadow: scrolled ? "0 2px 24px rgba(15,32,68,0.4)" : "none",
        }}
      >
        <div className="container-acm h-full flex items-center justify-between gap-8">
          ...
          <Link href="/" className="flex items-center gap-2.5 shrink-0 focus-visible:outline-orange-acm rounded" aria-label="ACM Hogares e Inversiones — Inicio">
            ...
            <span className="flex flex-col leading-none">
              ...
              <span className="font-sans text-[9px] tracking-[0.08em] uppercase text-cream/50 mt-0.5">
                Hogares <span className="text-orange-acm">e</span> Inversiones
              </span>
            </span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1" aria-label="Navegación principal">
            {NAV_LINKS.map(({ label, href }) => (
              <Link
                ...
                style={{ color: isActive(href) ? "#fff" : "rgba(251,248,244,0.7)" }}
              >
                {label}
                {isActive(href) && (
                  <motion.span
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            <a
              ...
              className="font-sans font-medium text-sm text-cream/75 hover:text-white transition-colors duration-200 flex items-center gap-2 px-3 py-1.5 rounded border border-white/20 hover:border-white/40"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              WhatsApp
            </a>
        ...
        {mobileOpen && (
          <>
            <motion.div
              ...
              transition={{ duration: 0.25 }}
              className="fixed inset-0 bg-navy-deep/70 backdrop-blur-sm z-[300] lg:hidden"
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 w-[min(320px,85vw)] bg-navy-deep z-[400] lg:hidden flex flex-col shadow-2xl border-l border-white/8"
            >
              ...
              <nav className="flex-1 overflow-y-auto px-2 py-4">
                {NAV_LINKS.map(({ label, href }) => (
                  <Link
                    ...
                    style={{
                      color: isActive(href) ? "#fff" : "rgba(251,248,244,0.75)",
              ...
              <div className="px-6 pb-8 flex flex-col gap-3 border-t border-white/8 pt-6">
                ...
                <a href={whatsappUrl()} target="_blank" rel="noopener noreferrer" className="btn-ghost w-full justify-center py-3.5 text-[15px]">
                  WhatsApp directo
                </a>
              </div>
            </motion.div>
